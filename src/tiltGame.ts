...
export default class TiltGame extends Phaser.Scene {
	...
	preload() {
		this.load.spritesheet('trampoline', 'assets/trampoline.png', {
			frameWidth: 32,
			frameHeight: 32
		});
		...
		this.load.image('1-bg1', 'assets/m3/1.png');
  		this.load.image('1-bg2', 'assets/m3/2.png');
  		this.load.image('1-bg3', 'assets/m3/3.png');
  		this.load.image('1-bg4', 'assets/m3/4.png');
  		this.load.image('1-bg5', 'assets/m3/5.png');

		this.load.image('2-bg1', 'assets/m1/1.png');
  		this.load.image('2-bg2', 'assets/m1/2.png');
  		this.load.image('2-bg3', 'assets/m1/3.png');
  		this.load.image('2-bg4', 'assets/m1/4.png');
  		this.load.image('2-bg5', 'assets/m1/5.png');

		this.load.image('3-bg1', 'assets/m6/1.png');
  		this.load.image('3-bg2', 'assets/m6/2.png');
  		this.load.image('3-bg3', 'assets/m6/3.png');
  		this.load.image('3-bg4', 'assets/m6/4.png');
  		this.load.image('3-bg5', 'assets/m6/5.png');
		...
		this.scene.scene.scale.on('resize', () => {
			...
			if(
				...
				// this.scoreText.setScale(this.scale.width / this.scoreText.width, this.scale.height / this.scoreText.height)
				this.maxScoreText.setPosition(this.scene.scene.scale.width - 250, 10);
	...
	create() {
		...
		this.maxScoreText = this.add.text(Number(this.scene.scene.scale.width) - 225, 10, `Max Score: ${maxScore}`, { fontFamily: 'Georgia, "Goudy Bookletter 1911", Times, serif', fontSize: '32px', color: '#fff' });
	...
	update(time: number, delta: number) {
		...
		if(!this.isGameOver) {
			this.updateScore(Math.floor(time / scoreFactor));
		} else {
			return;
		}
	...
	setupDeviceMotion() {
		...
		if (typeof DeviceMotionEvent.requestPermission === 'function') {
			...
			DeviceMotionEvent.requestPermission()
				...
				.catch(this.requestDeviceMotionPermissions);
		} else {
			this.setupTrampoline();
		}
	}
